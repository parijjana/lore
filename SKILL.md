...
1. **Brief the User**: Call `get_lore_protocol` to retrieve the professional briefing and the protocol payload.
...
5. **Re-triggering**: Remind the user they can re-run this setup at any time by calling `get_lore_protocol`.
...
1. Call `get_lore_ancestry` (depth 3) for any source that is already synthesized.
